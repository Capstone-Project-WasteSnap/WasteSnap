...
function preprocess(imageSource) {
  ...
  return new window.ort.Tensor("float32", float32Data, [
    1,
    targetSize,
    targetSize,
    3,
...
export async function analyzeImage(canvasElement) {
  if (!adaSampahSession || !jenisSampahSession) {
    throw new Error(
      "Model belum dimuat. Panggil loadModels() terlebih dahulu."
    );
  }
  ...
  const jenisSampahPred =
    jenisSampahResults[jenisSampahSession.outputNames[0]].data;
  ...
  if (type === "Organik") {
    return (
      <div style={recommendationContainerStyle}>
        ...
        <p>
          Sampah organik berasal dari bahan alami seperti sisa makanan, sayur
          dan buah busuk, kulit telur, daun kering, dan rumput. Sampah ini mudah
          membusuk dan terurai oleh mikroorganisme dalam waktu singkat.
        </p>
        ...
        <p style={{ marginTop: "16px" }}>
          Dengan memilah dan mengolah sampah organik, kita membantu menjaga
          lingkungan dan membuka peluang ekonomi hijau. Yuk, mulai dari rumah
          agar sampah organik jadi "emas hijau" yang bernilai!
        </p>
      </div>
    );
    ...
    return (
      <div style={recommendationContainerStyle}>
        ...
        <p>
          Sampah anorganik berasal dari bahan yang sulit terurai seperti
          plastik, logam, kaca, karet, dan kain sintetis. Karena susah membusuk,
          sampah ini harus kita kelola dengan bijak.
        </p>
        <p style={{ fontWeight: "bold", marginTop: "16px" }}>
        ...
        <p style={{ fontWeight: "bold", marginTop: "16px" }}>
          Contoh pemanfaatan ulang sampah anorganik:
        </p>
        <ul style={{ paddingLeft: "20px", listStyleType: "none", margin: 0 }}>
          <li>
            ♻️ Botol plastik jadi tas daur ulang, pot tanaman, atau bahan
            bangunan
          </li>
          ...
          <li>
            👜 Limbah tekstil dijadikan bantal, tas belanja, atau benang rajut
          </li>
        </ul>
        <p style={{ marginTop: "16px" }}>
          Dengan memilah dan memanfaatkan ulang sampah anorganik, kita
          mengurangi tumpukan sampah sekaligus mendukung ekonomi kreatif dan
          menjaga bumi tetap lestari. Yuk, mulai sekarang lebih bijak dalam
...
const PindaiPage = () => {
  ...
  useEffect(() => {
    ...
    const loadOnnxScript = () => {
      return new Promise((resolve, reject) => {
        ...
        script.onerror = () =>
          reject(new Error("Gagal memuat ONNX Runtime script"));
    ...
    const checkOrtAndLoadModels = async () => {
      if (retryCount >= maxRetries) {
        console.error(
          "Maksimum percobaan tercapai. ONNX Runtime tidak tersedia."
        );
        setErrorMessage(
          "Gagal memuat ONNX Runtime. Pastikan koneksi internet stabil dan coba refresh halaman."
        );
        return;
      }
      ...
      if (typeof window.ort === "undefined") {
        console.warn(
          "ONNX Runtime (ort) belum tersedia. Mencoba lagi dalam 100ms..."
        );
        retryCount++;
      ...
      try {
        ...
        if (!loaded) {
          setErrorMessage(
            "Gagal memuat model analisis. Silakan refresh halaman."
          );
        }
      } catch (error) {
        console.error("Error memuat model ML:", error);
        setErrorMessage(
          "Error memuat model. Pastikan koneksi internet stabil."
        );
      }
    };
    ...
    loadOnnxScript()
      ...
      .catch((error) => {
        ...
        setErrorMessage(
          "Gagal memuat ONNX Runtime. Pastikan koneksi internet stabil."
        );
      });
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setCameraError(
        "Tidak dapat mengakses kamera. Pastikan izin telah diberikan."
      );
      stopCamera();
    }
  }, [stopCamera]);
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        img.onload = resolve;
        img.onerror = () =>
          reject(new Error("Gagal memuat gambar yang diunggah."));
      });
                ...
                {analysisResult === "Bukan Sampah" ? (
                  <div style={{ marginBottom: "20px" }}>
                    ...
                    <div style={recommendationContainerStyle}>
                      ...
                      <p>
                        Silakan coba lagi dengan mengarahkan kamera lebih dekat
                        ke sampah yang ingin dipindai atau unggah gambar lain.
                      </p>
                    </div>
                  </div>
                ) : analysisResult ? (
